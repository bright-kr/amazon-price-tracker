"""
Amazon Price Tracker - Bright Data Web Scraper API
----------------------------------------------------
Track prices, availability, and product data from Amazon.

Docs  : https://docs.brightdata.co.kr/scraping-automation/web-scraper-api/overview
Info  : https://brightdata.co.kr/products/insights/price-tracker/amazon
"""
...
# Search for "Amazon" to find the matching Web Scraper (format: gd_xxxxxxxxxx).
...
if not DATASET_ID:
    raise EnvironmentError(
        ...
        "2. Search for 'Amazon' and copy the Web Scraper ID.\n"
...
def track_prices(urls: list[str], **kwargs) -> list[dict]:
    """
    Collect price data for a list of Amazon product URLs.
...
def discover_by_keyword(keyword: str, limit: int = 50, **kwargs) -> list[dict]:
    """
    Discover Amazon products by keyword search.
...
def discover_by_category(category_url: str, limit: int = 100, **kwargs) -> list[dict]:
    """
    Discover all products from a Amazon category page.

    Example:
        results = discover_by_category("https://amazon.com/s?k=headphones", limit=200)
    """
...
if __name__ == "__main__":
    ...
    output_file = "amazon_prices.json"
    ...
    # cat_results = discover_by_category("https://amazon.com/category/sample", limit=50)
